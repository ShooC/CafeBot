...
Ketik ${usedPrefix}oncs untuk bantuan
Bonus: ${poin} XP
    `.trim()
  conn.logoquiz[id] = [
    await conn.sendButtonImg(m.chat, await (await fetch(json.img)).buffer(), caption, wm, 'Help/Bantuan', '.oncs', m)
    ,
    json, poin,
    setTimeout(async () => {
      if (conn.logoquiz[id]) await conn.sendButton(m.chat, `Yahh aktu habis!\nJawabannya adalah *${json.jawaban}*`, wm, 'Main Lagi', '.logoquiz', conn.logoquiz[id][0])
