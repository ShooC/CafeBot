...
module.exports = {
        ...
        try {
            ...
            try {
                ...
                if (user) {
                    ...
                    if (!isNumber(user.lastprostitusi)) user.lastprotistusi = 0
                    ...
                    if (!('prostitusi' in user)) user.prostitusi = false
                       ...
                       let pp = 'https://telegra.ph/file/58acbba00bf7570860e61.png'
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome @user').replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc.toString()) :
                                (chat.sBye || this.bye || conn.bye || 'Byee @user!')).replace('@user', '@' + user.split('@')[0])
                                this.send2ButtonImg(id, pp, text, "Cafe Id Bot\nBot Whatsapp", "Hello", "hii", "Tampilan Menu", ".menu", fake, { contextInfo: { externalAdReply :{
                                ...
                                title: action === 'add' ? 'Selamat datang kak' : 'Yahh keluar',
                                }}})
                                }
                    }
                }
    ...
    async groupsUpdate(groupsUpdate, fromMe, m) {
        ...
        for (let groupUpdate of groupsUpdate) {
            ...
            //console.log('=============\n\ngroupsUpdate \n\n============\n\n' + await groupUpdate)
            if (!text) continue
            await this.sendButton(id, text, wm, 'Off Fitur Ini', `.off detect`, fake, { contextInfo: { mentionedJid: this.parseMention(text) }, mentions: await this.parseMention(text) })
